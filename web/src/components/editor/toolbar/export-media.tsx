    ...
    if (activeLayer?.publicId) {
      try {
        ...
        const res = await fetch(
          `/api/editor/download?publicId=${activeLayer.publicId}&quality=${selected}&resource_type=${activeLayer.resourceType}&format=${activeLayer.format}&url=${activeLayer.url}`
        );
        if (!res.ok) {
  ...
  return (
    <Dialog>
      ...
      <DialogContent>
        ...
        <Button onClick={handleDownload} disabled={dowloading}>
